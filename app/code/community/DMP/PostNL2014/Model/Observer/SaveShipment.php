 ...
 */
class DMP_PostNL2014_Model_Observer_SaveShipment
{
    /**
    ...
    public function saveConsignmentOption(Varien_Event_Observer $observer)
    {
        ...
        if (false !== $helper->getPgAddress($shipment->getOrder())
            && (!isset($registryOptions['shipment_type']) ||
                $registryOptions['shipment_type'] != DMP_PostNL2014_Model_Shipment::TYPE_NORMAL
            )
        )
        {
            return $this;
        ...
        while ($i <= $multicolloAmount) {
            ...
            if ($barcode) {
                $carrierCode = DMP_PostNL2014_Model_Shipment::POSTNL_CARRIER_CODE;
