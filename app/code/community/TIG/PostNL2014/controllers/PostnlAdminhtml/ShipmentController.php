 ...
 */
class TIG_PostNL2014_PostnlAdminhtml_ShipmentController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Used module name in current adminhtml controller.
     */
    protected $_usedModuleName = 'TIG_PostNL2014';
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('admin/tig_postnl');
    }
    ...
    public function massCreateShipmentsAction()
    {
        ...
        try {
            ...
            foreach ($orderIds as $orderId) {
                ...
                } catch (TIG_PostNL2014_Exception $e) {
    ...
    public function createConsignmentAction()
    {
        ...
        //get post variables
        $selectedConsignmentOptions = $this->getRequest()->getPost('tig_postnl');
        ...
        try{
            ...
            if(!empty($shipmentId)){
                ...
                $postNLShipment = Mage::getModel('tig_postnl/shipment')->setShipmentId($shipmentId);
        ...
        if (!empty($selectedConsignmentOptions['shipment_type'])) {
            ...
            try {
                ...
                if($helper->getPgAddress($shipment->getOrder()) && ($shipmentType != TIG_PostNL2014_Model_Shipment::TYPE_NORMAL && $shipmentType != 'default')){
    ...
    public function massPrintLabelsAction()
    {
        ...
        Mage::register('tig_postnl_consignment_options', array(
         ...
         *
         * @var TIG_PostNL2014_Model_Shipment $shipment
         */
        ...
        foreach ($shipments as $shipment) {
            try {
                if (!$shipment->hasConsignmentId()) {
                    ...
                    if($helper->getPgAddress($shipment->getOrder()) && $type != TIG_PostNL2014_Model_Shipment::TYPE_NORMAL && $type != 'default'){
                        $shipment_url = Mage::helper('adminhtml')->getUrl('adminhtml/sales_order_shipment/view',array('shipment_id' => $shipment->getShipment()->getId()));
                        throw new TIG_PostNL2014_Exception(
                            $helper->__('The selected shipment type cannot be used. Pakjegemak shipments can only be created with the normal shipment type.<br/> The Magento shipment has been created without a PostNL shipment, select a different shipment type or go to the shipment page to create a single PostNL shipment. <a target="_blank" href="%s">View shipment</a>',$shipment_url),
                            'MYPA-0023'
                    ...
                    if (Mage::registry('tig_postnl_consignment_options')) {
                        $consignmentOptions = array_merge(
                            $consignmentOptions,
                            Mage::registry('tig_postnl_consignment_options')
                        );
     ...
     *                                         Mage_Sales_Model_Shipment or TIG_PostNL2014_Model_Shipment objects.
    ...
    protected function _loadAndCheckShipments($shipmentIds, $loadPostNLShipments = false, $throwException = true, $keyIsConsignmentId = true)
    {
        ...
        foreach ($shipmentIds as $shipmentId) {
             ...
             * Load the shipment.
             *
             * @var Mage_Sales_Model_Order_Shipment|TIG_PostNL2014_Model_Shipment|boolean $shipment
             */
     ...
     *
     * @return boolean|Mage_Sales_Model_Order_Shipment|TIG_PostNL2014_Model_Shipment
     */
    protected function _loadShipment($shipmentId, $loadPostNLShipment)
    {
         ...
         */
        if (!Mage::helper('tig_postnl')->shippingMethodIsPostNL($shippingMethod) || $shipment->getShipment()->getOrder()->getIsVirtual()) {
            return false;
        }
     ...
     * @param int $shipmentId
     *
     * @return TIG_PostNL2014_Model_Shipment
     */
    protected function _getPostNLShipment($shipmentId)
    {
        $postNLShipment = Mage::getModel('tig_postnl/shipment')->load($shipmentId, 'shipment_id');
     ...
     *
     * @return int|TIG_PostNL2014_Exception
     *
     *
     * @throws TIG_PostNL2014_Exception
     */
