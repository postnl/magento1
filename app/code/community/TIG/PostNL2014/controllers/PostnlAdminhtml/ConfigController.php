...
class TIG_PostNL2014_PostnlAdminhtml_ConfigController extends Mage_Adminhtml_Controller_Action
{
    /**
     * @return bool
     */
    ...
    public function downloadLogsAction()
    {
        ...
        try
        {
            $zip = Mage::getModel('tig_postnl/adminhtml_support_logs')->downloadLogs();
        }
        catch (Exception $e)
        {
            Mage::getSingleton('core/session')->addError($helper->__('The log files cannot be downloaded.'));

            $this->_redirect('adminhtml/system_config/edit', array('section' => 'tig_postnl'));
            return $this;
        }
        if(empty($zip))
        {
