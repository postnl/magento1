 ...
 */
class TIG_PostNL2014_Model_Checkout_Service
{
    /**
     * Save in checkout PostNL shipping method
     */
    public function savePostNLShippingMethod()
    {
        $request = Mage::app()->getRequest();
        if($request->isPost()){

            $addressHelper = Mage::helper('tig_postnl/addressValidation');
            ...
            if ($address['country'] !== 'NL' && $address['country'] !== 'BE'){
                $quote->setPostnlData(null)->save();
                return true;
            }

            /**
             * If shipping method is postnl
             */
            if (strpos($request->getPost('shipping_method', ''), 'postnl') !== false) {
                ...
                $quote->setPostnlData(json_encode($data))->save();
        ...
        if ($quote) {
            if ($quote != '{}') {
                ...
                foreach ($addresses as $address) {
                    if ($address->getAddressType() == TIG_PostNL2014_Helper_Data::PG_ADDRESS_TYPE) {
                        $address->delete();
                    }
                }
            }
