 ...
 */
class TIG_PostNL2014_Block_Adminhtml_Widget_Grid_Column_Renderer_Shipment_ShippingStatus
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Text
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        if (!Mage::helper('tig_postnl')->shippingMethodIsPostNL($shippingMethod)) {
            return '';
        }
        ...
        $barcode = $row->getData(self::BARCODE_COLUMN);
            if (!empty($barcode)) {
                $barcodeUrl = Mage::helper('tig_postnl')->getBarcodeUrl($barcode, $destinationData, false, true);
