 ...
 */
class TIG_PostNL2014_Block_Adminhtml_Widget_Grid_Column_Renderer_ShippingStatus
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Text
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        $helper = $this->helper('tig_postnl');
        $html = '';

        $order = Mage::getModel('sales/order')->load($row->getId());
        $postNLShipments = Mage::getModel('tig_postnl/shipment')
            ->getCollection()
        ...
        if (count($postNLShipments) > 0) {
            ...
            foreach ($postNLShipments as $postNLShipment) {
                ...
                $barcodeUrl = $helper->getBarcodeUrl($postNLShipment->getBarcode(), $destinationData, false);
                if ($postNLShipment->getBarcode())
                    $html .= "<a href='{$barcodeUrl}' target='_blank'>{$postNLShipment->getBarcode()}</a>";
                ...
                $html .= '&nbsp; <small>' . $this->__('status_' . $postNLShipment->getStatus()) . "</small>";
            }
